        ...
        try {
            "okhttp3.RealCall".toClass().hook {
                injectMember {
                    ...
                    afterUnit {
                        try {
                            val response = result ?: return@afterUnit
                            val requestField = response.javaClass.getDeclaredMethod("request")
                            val request = requestField.invoke(response) ?: return@afterUnit
                            val urlMethod = request.javaClass.getDeclaredMethod("url")
                            val url = urlMethod.invoke(request)?.toString() ?: return@afterUnit
